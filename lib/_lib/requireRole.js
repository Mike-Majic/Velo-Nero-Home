...
export async function requireAuth(req) {
  ...
  try {
    ...
    return { ok: true, user: { ...payload, role: appRole(profile.role || payload.role, payload.email), name: profile.name || profile.username || payload.name || "", picture: profile.picture || payload.picture || "" } };
