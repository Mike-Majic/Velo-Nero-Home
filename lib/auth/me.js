...
export default async function handler(req, res) {
  ...
  try {
    ...
    const p = rows?.[0] || {};
    return res.status(200).json({ ok: true, authenticated: true, user: { email: payload.email, name: p.name || p.username || payload.name || "", picture: p.picture || payload.picture || "", role: appRole(p.role || payload.role, payload.email) } });
